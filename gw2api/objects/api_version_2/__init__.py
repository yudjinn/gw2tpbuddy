...
class BaseAPIv2Object(BaseAPIv2Object):
    '''Extends base API handler to automatically handle pagination and id parameters'''
...
class CommerceExchangeCoins(BaseAPIObject):
    '''Returns current coins to gems exchange rate'''
...
class CommerceTransactions(BaseAPIv2Object):
    '''Returns info on an accounts past and current TP transactions'''
    @property
    def session(self):
        return self.session
